...
# Caches MUST be on a writable, bound path. Use project_462000963.
...
echo "Installing dispatcher and ninja in container..."
SING_EXEC "$PIP_IN_IMG install --user --upgrade 'git+https://github.com/LumiOpen/dispatcher.git' ninja"
echo "Singularity environment setup complete."
...
# Start dispatcher server in container
SING_EXEC "
  set -eux
...
(echo >/dev/tcp/$DISPATCHER_SERVER/${DISPATCHER_PORT}) >/dev/null 2>&1 || { echo "[ERROR] dispatcher server did not start"; exit 1; }
echo "Server is up."
...
srun -l singularity exec --rocm --cleanenv "${BINDS[@]}" "$IMG" bash --noprofile --norc -c '
  ...
  # ensuring builds happen in a writable directory.
