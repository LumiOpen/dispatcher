...
setup_apptainer_environment() {
    # HuggingFace
    export APPTAINERENV_HF_HOME="/project/cache"
    export APPTAINERENV_HUGGINGFACE_HUB_CACHE="/project/cache/hub"
    export APPTAINERENV_HF_HUB_DISABLE_XET=1
    ...
    export APPTAINERENV_LAUNCHER_STORAGE_PRJ="$LAUNCHER_STORAGE_PRJ"
...
run_sing_bash() {
    ...
    # Pass SLURM variables explicitly via --env (APPTAINERENV_* passthrough is unreliable)
